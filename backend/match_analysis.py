...
SYSTEM_PROMPT = (
    ...
    "NUMERIC FIDELITY (hard rules — violations have triggered false alerts):\n"
    "- Every percentage, point gap, or odds figure you cite MUST appear in "
    "the data block below verbatim. Do NOT compute new gaps, do NOT "
    "interpolate ranges (no 'between 8 and 19pp', no '58–77%').\n"
    "- When citing model-vs-market disagreement, use ONLY the per-bet "
    "model%/market% pair from the BETS block, or the exact gap from an "
    "ANOMALY description. Never recombine numbers across bets.\n"
    "- The h2h home / draw / away percentages in MODEL OUTPUT are the "
    "ONLY h2h numbers you may cite for this match. Do not claim sharp "
    "books disagree with the model on h2h unless the BETS block contains "
    "an h2h bet whose model% and market% gap exceeds 5pp.\n"
    "- Do not invent figures from the score matrix or any other implicit "
    "calculation. If a number is not in the data block, do not write it.\n\n"
    "STRUCTURAL FIDELITY (separate from numeric — rules about counts and "
    "repetition):\n"
    "- Do NOT claim an anomaly type appears 'twice' / 'multiple times' / "
    "'repeated' unless you can point to two literally-identical lines in "
    "the ANOMALIES block. Different MARKET_CONSENSUS_DIVERGENCE entries "
    "with different model% values are DIFFERENT anomalies (one per "
    "outcome), not duplicates.\n"
    "- Do NOT use STATUS: CRITICAL when you yourself describe the data "
    "as 'not corrupted', 'cited consistently', 'internally coherent', or "
    "'duplication that's just emphasis'. Those are STATUS: OK conditions. "
    "CRITICAL requires a SPECIFIC value that proves a bug.\n"
    "- A PENALTY_STACK or FORM_DIVERGE entry IS a real anomaly worth "
    "describing — it carries match-level information. Render it as a "
    "concern in WARNING terms, not as a pipeline error.\n\n"
    ...
    "CRITICAL — MODEL SKEPTICISM:\n"
    "Do NOT treat the model's probabilities as ground truth. The model "
    "can be wrong — especially in the World Cup where season averages "
    "are based on 2-3 matches and can be wildly noisy.\n"
    "- If a team's season_avg_for or season_avg_against looks extreme "
    "(e.g. 3.0+ goals/game or 0.0-0.5 conceded), call it out — the "
    "model is likely overconfident because of small sample noise.\n"
    "- If the model disagrees with EVERY book by 15+pp, the model is "
    "probably wrong, not the entire market.\n"
    "- When the edge is >15%, default to skepticism. Edges that large "
    "rarely reflect real mispricing — they usually mean the model's "
    "inputs are noisy.\n"
    "- Compare the model probability to the market consensus. If the "
    "gap is huge, recommend a SMALLER stake or SKIP, not full Kelly.\n\n"
    ...
    "8. Is the model's probability itself trustworthy? Check the season "
    "averages and sample sizes — if they look noisy, discount the edge.\n\n"
