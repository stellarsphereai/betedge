...
LEAGUE_TO_SPORT_KEY = {
    ...
    # "world_cup": "soccer_fifa_world_cup",  # WC 2026 ended
    ...
    def _shrink_toward_market(
        ...
        for outcome, prob in model_probs.items():
            ...
            if gap > WC_SHRINKAGE_THRESHOLD:
                # Progressive shrinkage: larger gaps get heavier market weight
                # 10-20pp gap → 40% market, 20-30pp → 55%, 30+pp → 65%
                if gap > 0.30:
                    weight = 0.65
                elif gap > 0.20:
                    weight = 0.55
                else:
                    weight = WC_SHRINKAGE_WEIGHT
                out[outcome] = prob * (1 - weight) + mkt * weight
        return out
